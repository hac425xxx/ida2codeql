...
class IdaTypeResolver:
    ...
    def parse_struct(self, typs, sid):

        print("parse {} of sid 0x{:x}".format(typs, sid))
        struc = idaapi.get_struc(sid)
        members = []
        for i in range(struc.memqty):
            member: idaapi.member_t = struc.get_member(i)
            mname = idaapi.get_member_name(member.id)
            mtif: idaapi.tinfo_t = idaapi.tinfo_t()
            idaapi.get_member_tinfo(mtif, member)
            m = {
                "name": mname,
                "type": self.parse_tinfo(mtif),
            }
            members.append(m)
    ...
    def parse_tinfo(self, ti: idaapi.tinfo_t):
        ...
        if self.basictype_map.get(full_type):
            ...
            if sid == idaapi.BADADDR:
                ...
                print("parse member: {}".format(typs))
                r = self.parse_struct(typs, sid)
        elif ti.is_ptr():
